...
const SectionTitle: FC<Props> = (props) => {
  const {title: pageTitle, description: pageDescription} = props
      ...
      {pageBreadcrumbs && pageBreadcrumbs.length > 0 && (
        <>
          ...
          <ul className='breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0'>
            {Array.from(pageBreadcrumbs).map(
              (item, index) =>
                !item?.isLastCrumb && (
                  <li
            ...
            <li className='breadcrumb-item text-dark'>
              {pageBreadcrumbs.filter((r) => r.isLastCrumb).length > 0
                ? pageBreadcrumbs.filter((r) => r.isLastCrumb)[0].title
                : pageTitle}
            </li>
          </ul>
        </>
